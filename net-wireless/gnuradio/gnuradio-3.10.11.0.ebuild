...
if [[ ${PV} =~ "9999" ]]; then
	...
	#New qtwebengine for now optional dep for new qt grc version that will eventually be default. -x86, -arm, -riscv
	KEYWORDS="~amd64"
fi
...
RESTRICT="!test? ( test )"
#RESTRICT="test" # tries to import installed libraries, not freshly built ones.
...
RDEPEND="${PYTHON_DEPS}
	...
	qt5? (
		...
		grc? (
			$(python_gen_cond_dep 'dev-python/qdarkstyle[${PYTHON_USEDEP}]')
			$(python_gen_cond_dep 'dev-python/pyqtwebengine[${PYTHON_USEDEP}]')
			$(python_gen_cond_dep 'dev-python/pyqt5[svg,${PYTHON_USEDEP}]')
		)
		dev-qt/qtcore:5
		dev-qt/qtgui:5
		x11-libs/qwt:6=[qt5(+)]
