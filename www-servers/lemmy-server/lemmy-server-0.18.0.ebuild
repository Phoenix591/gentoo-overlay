...
https://github.com/LemmyNet/lemmy/archive/refs/tags/${PV}.tar.gz -> ${P}.tar.gz
https://drive.google.com/uc?export=download&id=1CHsJtRFgFgnyat5BdQ8W-BZ1GiENCyNF -> lemmy-translations-${PV}.tar.xz"
...
#PATCHES="${FILESDIR}/fix-translation-build-0.18.0.patch"
...
src_prepare() {
	default
	pushd "${WORKDIR}/cargo_home/gentoo/activitypub_federation-0.4.4/" || die
	eapply "${FILESDIR}/0.18.0-http-expiration.patch" || die
	popd
	if [ "${PV}" != "9999" ]; then
		echo "pub const VERSION: &str = \"${PV}\";" > "crates/utils/src/version.rs"
	else
